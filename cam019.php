...
// Sistema			: CAMELIA
// Programa			: CAM019.PHP
// Descripcion		: Agrega instituciones al sistema.
...
<head><title>Ingreso de Instituciones</title>
<link href="ecocss/vvppcss.css" type="text/css" rel="stylesheet" />
	...
	function vRut(){
   	/*----------------------------------*/
		var variable = Form1.cmd_rutins.value.substr(0,Form1.cmd_rutins.value.indexOf("-"));
		var digit = Form1.cmd_rutins.value.substr(Form1.cmd_rutins.value.indexOf("-")+1).toUpperCase();
		
		if ( validaRut(variable,digit) == false )
			  { alert("Rut Invalido") 
			}
		Form1.cmd_rutins.value = Form1.cmd_rutins.value.toUpperCase();
...
if (!theForm) {
    ...
    <table id="Table1" border="0" cellpadding="0" cellspacing="0" style="z-index: 101; left: 1px; position: absolute; top: 2px; height: 300px; width: 606px;" >
      <tr>
          ...
          <tr>
        <td align="top" width="584"> <table width="599" height="175" border="0" cellpadding="0" cellspacing="0" class="texto13" id="Table2"
            ...
            <?php 
			if (strlen(trim($codins_w)) == 0)
			 {
			   echo '<td style="width: 94px; height: 18px"> <input name="cmd_codins" type="text" maxlength="03" id="cmd_codins" class="input-normal" style="text-transform: uppercase;width:56px;" onKeypress="if (event.keyCode < 48 || event.keyCode > 57) event.returnValue = false;" value="'.$_SESSION["codins_s"].'" onchange="this.value = this.value.toUpperCase();"/> ';
			  }
			 else
			   { 
			...
			<tr> 
              ...
              <td width="365" style="width: 314px; height: 18px"> <input name="cmd_rutins" type="text" maxlength="13" id="cmd_rutins" class="input-normal" style="text-transform: uppercase; width:76px;" onchange="vRut()" value="<?php echo $rutins_w; ?>" /> 
            ...
            <tr> 
              <td style="width: 95px; height: 3px"> &nbsp;Fax</td>
 		        <td style="width: 314px; height: 3px"> <input name="cmd_fax" type="text" maxlength="10" id="cmd_fax" class="input-normal" style="width:176px;" value="<?php echo $fax_w; ?>" /> 
                &nbsp; </td>
            </tr>
            <tr> 
		...
		if (!mysqli_query($link,$sql1)) 
			{ $error = mysqli_error($link);
		     ...
		     if (mysqli_errno($link) == 1062)
			    {
				$merror = "La institución ya existe....." ;
				} 
		   ...
		   {
			echo "<script type=\"text/javascript\">
				alert('La Institución: \' $codins_w \' ha sido registrada de manera satisfactoria.');
				</script>";
			mysqli_close($link);
			}	
