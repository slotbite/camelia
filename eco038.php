...
// Sistema			: ECO
// Programa			: ECO038.PHP
// Descripcion		: Ingreso Exámen Eco Arterial Normal.
...
<head><title>EcoDoppler Arterial Extremidades Inferiores</title>
<link href="ecocss/vvppcss.css" type="text/css" rel="stylesheet" />
   ...
   //       window.open('eco_autoriza.php?PAGINA=' + 'eco002.php' ,'Paciente','width=650, height=550, status=yes, resizable=no , menubar=yes, scrollbars=yes, location=no, top=150, left=220').focus();
	...
	/*-----------------------------------*/
        ...
        window.open('eco_autoriza.php?PAGINA=' + 'eco038_i.php?CODIGO=' + cod,'eco038_i','width=650, height=550, status= no, resizable= yes, menubar=yes, scrollbars=yes, location=no, top=150, left=220').focus();
...
if (isset($_POST['txt_detalle'])){
	...
	$detalle_w  = "Se exploran arterias de ambas extremidades inferiores con transductor lineal de 7.5 Mhz con análisis doppler color.
	
Todas las arterias exploradas es decir ambas arterias femorales, comunes, superficiales y comunes, poplíteas, tibiales anteriores, tibiales posteriores y pedía son de morfología y de aspecto normal con lúmenes conservados y fino engrosamiento sub intimal a nivel poplíteo izquierdo y tibial posterior distal bilateral no estenosente.

Al examen doppler color flujos conservados trifásicos, con velocidades en rango normal.
...
if (isset($_POST['txt_conclusion'])){
	...
	$conclusion_w = "EXAMEN DENTRO DE LIMITES NORMALES.
";
...
if ($_GET["CODIGO"] > 0)
	...
	$consulta="call ECO_PSEL_ECOARTERIALNORMAL('".$_GET["CODIGO"]."',null,null,null,null,null)";
    ...
    <table id="Table1" border="0" cellpadding="0" cellspacing="0" style="z-index: 101; left: 16px; position: absolute; top: 6px; height: 498px; width: 623px;" width="510">
      <tr>
        ...
        <td style="height: 24px" valign="bottom"> <span id="Label1" class="texto18"> 
          ECODOPPLER ARTERIAL EXTREMIDADES INFERIORES</span></td>
            </tr>
            <tr>
                <td style="background-image: url(ivvpp0002.jpg); width: 34px; height: 24px">
                </td>
...
//		$sql1 = "call ECO_PUPD_PACIENTES('I','".$rut_w."','".$nombre_w."','".$apepat_w."','".$apemat_w."',null,null,null,null,null,null,null,null,null)";
		$sql1 = "call ECO_PUPD_ECODOPARTERIALNORMAL('".$modo_w."','".$codigo_w."','".$rutpac_w."',STR_TO_DATE('".$fecha_w."','%d/%m/%Y'),";
