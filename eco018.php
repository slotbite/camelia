...
// Sistema			: ECO
// Programa			: ECO018.PHP
// Descripcion		: Ingreso Exámen EcoDoppler Venoso Una Pierna.
...
<head><title>EcoDoppler Venoso Una Pierna</title>
...
//          window.open('eco_autoriza.php?PAGINA=' + 'eco002.php' ,'Paciente','width=650, height=550, status=yes, resizable=no , menubar=yes, scrollbars=yes, location=no, top=150, left=220').focus();
	...
	/*-----------------------------------*/
        ...
        window.open('eco_autoriza.php?PAGINA=' + 'eco018_i.php?CODIGO=' + cod,'eco018_i','width=650, height=550, status= no, resizable= yes, menubar=yes, scrollbars=yes, location=no, top=150, left=220').focus();
...
if (isset($_POST['txt_examen'])){
	$examen_w  = trim($_POST['txt_examen']);
	}
else{
	$examen_w  = "Se utiliza una sonda doppler color lineal de 7 Mhz de alta capacidad de resolución.  Se examina en forma continua los troncos venosos, y se toma fotos B/N y color sólo en algunos sitios de interés.
Se encuentra la vena femoral común, superficial y profunda, la vena poplítea y las venas tibiales posteriores de calibre amplio, de paredes finas y compresibles, permeables en forma espontánea en las curvas de doppler venoso y en el doppler color, sin imágenes ecogénicas intraluminales. Se observa buena capacidad de aumento de flujo venoso frente a la compresión distal y buena competencia valvular con la maniobra de Valsalva y frente a la compresión proximal.
El cayado de la vena safena interna aparece de calibre normal, compresible y permeable en el doppler color, con buena continencia valvular frente a la maniobra de Valsalva.  La vena safena externa está permeable y compresible, con competencia valvular frente a la compresión proximal.  Siguiendo con la sonda doppler las venas tibiales posteriores en la cara interna de la pierna no observo venas perforantes insuficientes o dilatadas.";
	}
if (isset($_POST['txt_conclusion'])){
	$conclusion_w  = trim($_POST['txt_conclusion']);
	}
else{
	$conclusion_w = "Sistema venoso profundo permeable de la extremidad inferior derecha.
Sin signos de trombosis venosa profunda o superficial actual.";
	}
...
if ($_GET["CODIGO"] > 0)
	...
	$consulta="call ECO_PSEL_ECODEPVENPIERNA('".$_GET["CODIGO"]."',null,null,null,null,null)";
    ...
    <table id="Table1" border="0" cellpadding="0" cellspacing="0" style="z-index: 101; left: 16px; position: absolute; top: 6px; height: 498px; width: 623px;" width="510">
      <tr>
                ...
                <td style="height: 24px" valign="bottom">
                    <span id="Label1" class="texto18"> ECODOPPLER VENOSO UNA PIERNA</span></td>
            </tr>
            <tr>
                <td style="background-image: url(ivvpp0002.jpg); width: 34px; height: 24px">
                </td>
          ...
          <tr>
            ...
            <tr> 
			  ...
			  <textarea name="txt_examen" cols="80" rows="20" class="input-normal" ><?php echo $examen_w; ?></textarea>
                </td>
            </tr>
            <tr> 
              <td background="ivvpp0003.jpg" colspan="2" height="2" style="width: 381px"> 
...
//		$sql1 = "call ECO_PUPD_PACIENTES('I','".$rut_w."','".$nombre_w."','".$apepat_w."','".$apemat_w."',null,null,null,null,null,null,null,null,null)";
		$sql1 = "call ECO_PUPD_ECODOPVENPIERNA('".$modo_w."','".$codigo_w."','".$rutpac_w."',STR_TO_DATE('".$fecha_w."','%d/%m/%Y'),";
        $sql1 = $sql1."'".$antecedentes_w."','".$examen_w."','".$conclusion_w."','".$rutmed_w."',@cod_w)";
