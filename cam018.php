...
// Sistema			: CAMELIA
// Programa			: CAM018.PHP
// Descripcion		: Mantención de Instituciones.
...
<HTML>
	<head>
        ...
        imagen.src = "ivvpp/ivvpp0020.gif";
			...
			function f_cliente(){
			/*-------------------------*/
	              window.open('eco_autoriza.php?PAGINA=' + 'cam019.php?COD=' ,'cam019','width=450, height=350, status= no, resizable= no, menubar=no, scrollbars=yes, location=no, top=150, left=220').focus();
			}
			
			function Modificar_Vtna()
	          {
 	              window.open('eco_autoriza.php?PAGINA=' + 'cam019.php?COD=' + document.getElementById('hdCodigo').Value ,'cam019','width=450, height=350, status= no, resizable= yes, menubar=no, scrollbars=yes, location=no, top=150, left=220').focus();
	          }

			function Eliminar_Vtna()
			/*-------------------------*/
	          {
 	              window.open('eco_autoriza.php?PAGINA=' + 'cam019_e.php?COD=' + document.getElementById('hdCodigo').Value ,'cam019_e','width=450, height=350, status= no, resizable= yes, menubar=no, scrollbars=yes, location=no, top=150, left=220').focus();
	          }
...
if ($_POST) {
    $_SESSION['codins_s'] = $_POST['cmd_codins'];
}
?>
		<TABLE id="Table1" cellSpacing="0" cellPadding="0" width="530" border="0">
			<TR>
				<td vAlign="bottom" width="500">&nbsp;
					<span id="Label1" class="texto18">INSTITUCIONES</span></td>
			</TR>
			<TR>
				<td align="left" width="500" height="6">
					<TABLE class="texto13" id="Table2" cellPadding="0" width="615" border="0">
        <TR> 
						<td width="85" style="WIDTH: 70px; HEIGHT: 12px"></td>
						<td width="224" style="WIDTH: 224px; HEIGHT: 12px"></td>
						<td width="151" style="WIDTH: 70px; HEIGHT: 12px"></td>
						<td width="145" style="HEIGHT: 12px"></td>
					  </TR>
					  <TR> 
						<td style="WIDTH: 85px; HEIGHT: 3px">&nbsp;Cód. Institución</td>
						<td style="WIDTH: 224px; HEIGHT: 3px"><input name="cmd_codins" type="text" id="cmd_codins" class="input-normal" style="width:76px;" value="<?php if (isset($_SESSION['codins_s'])){ echo $_SESSION['codins_s']; } ?>"/></td>
...
if ($_POST) 
{
 if (isset($_POST["cmd_buscar"])) 
    {
    $cod_w = $_POST["cmd_codins"];
	
	//Conexion con la base
	$link = mysqli_connect(DB_SERVER,DB_USER,DB_PASS,DB_NAME);
	
	//Ejecutamos la sentencia SQL
	$consulta="call cam_psel_instituciones('".$cod_w."',null)";

	$result=mysqli_query($link,$consulta);
	
	echo '<TR>';
	echo '	<td vAlign="top" align="center" width="400" height="145">';
    echo '		<P><table class="link13" cellspacing="0" cellpadding="3" align="Left" rules="rows" border="1" id="dgrid" style="border-color:#E7E7FF;border-width:1px;border-style:None;height:20px;width:400px;border-collapse:collapse;">';
	...
	while ($row=mysqli_fetch_array($result))
		...
		echo '<td></td><td>';
		echo '<input type="radio" name="rbnRut" value="'.$row["codins"].'" onClick="Remueve_Opcion2(&quot;'.$row["codins"].'&quot;,&quot;'.$opc_w.'&quot;)"/>';
		echo '<label for="rbnRut">'.$row["codins"].'</label>';
    ...
    echo '</table></P>	</td> </TR>';
	echo '			<TR>';
	echo '				<td vAlign="middle" align="center" width="400" height="10">';
	echo '					<TABLE id="Table3" style="WIDTH: 400px; HEIGHT: 25px; background-color: whitesmoke;" cellSpacing="1" cellPadding="1" width="400"';
