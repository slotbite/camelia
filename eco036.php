...
// Sistema			: ECO
// Programa			: ECO032.PHP
// Descripcion		: Ingreso Exámen EcoDoppler Extremidades Inferiores.
...
<link href="ecocss/vvppcss.css" type="text/css" rel="stylesheet" />
 ...
 //         window.open('eco_autoriza.php?PAGINA=' + 'eco002.php' ,'Paciente','width=650, height=550, status=yes, resizable=no , menubar=yes, scrollbars=yes, location=no, top=150, left=220').focus();
	...
	/*-----------------------------------*/
        ...
        window.open('eco_autoriza.php?PAGINA=' + 'eco036_i.php?CODIGO=' + cod,'eco036_i','width=650, height=550, status= no, resizable= yes, menubar=yes, scrollbars=yes, location=no, top=150, left=220').focus();
...
if (isset($_POST['txt_detalle'])){
	...
	$detalle_w  = "Se exploran venas de ambas extremidades inferiores con transductor lineal 7.5  Mhz.   Con análisis doppler color.
	
Las Venas Femorales comunes al igual que los Cayados Safena Interna son de aspecto normal, con lúmenes compresibles y superficies intímales sin defectos.  Al examen Doppler  Color los flujos son bifásicos continentes con maniobra de valsalva.

Las Venas Femorales Superficiales presentan igual aspecto y morfología con lumen y flujo conservado.

Las venas poplíteas están permeables compresibles, con flujos conservados, competentes con maniobra de compresión proximal.  Midieron ……  y ………. mm de calibre proximal.  Cayado Safena Externa competente bilateral.

Vena gemelar derecha e izquierda permeables, midieron ……. y …… mm de calibre.

Hacia distal las venas Tibiales Posteriores  están permeable, compresibles, continentes con maniobra de compresión proximal.  Midieron …….. y…….mm de calibre a ambos lados.

No se visualizan venas perforantes en este examen.
";
	}
if (isset($_POST['txt_conclusion'])){
	...
	$conclusion_w = "PERMEABILIDAD VENOSA PROFUNDA BILATERAL.
...
if ($_GET["CODIGO"] > 0)
	...
	$consulta="call ECO_PSEL_ECODOPEXTINFERIOR('".$_GET["CODIGO"]."',null,null,null,null,null)";
    ...
    <table id="Table1" border="0" cellpadding="0" cellspacing="0" style="z-index: 101; left: 16px; position: absolute; top: 6px; height: 498px; width: 623px;" width="510">
      <tr>
        ...
        <td style="height: 24px" valign="bottom"> <span id="Label1" class="texto18"> 
          ECODOPPLER COLOR EXTREMIDADES INFERIORES</span></td>
            </tr>
            <tr>
                <td style="background-image: url(ivvpp0002.jpg); width: 34px; height: 24px">
                </td>
...
//		$sql1 = "call ECO_PUPD_PACIENTES('I','".$rut_w."','".$nombre_w."','".$apepat_w."','".$apemat_w."',null,null,null,null,null,null,null,null,null)";
		$sql1 = "call ECO_PUPD_ECODOPEXTINFERIOR('".$modo_w."','".$codigo_w."','".$rutpac_w."',STR_TO_DATE('".$fecha_w."','%d/%m/%Y'),";
        $sql1 = $sql1."'".$detalle_w."','".$conclusion_w."','".$rutmed_w."',@cod_w)";
