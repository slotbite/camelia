...
// Sistema			: CAMELIA
// Programa			: CAM022.PHP
// Descripcion		: Mantención parámetros del sistema.
...
<head><title>Mantención de Parámetros</title>
...
{ 
	...
	$consulta="call cam_psel_parametros()";
    ...
    <table id="Table1" border="0" cellpadding="0" cellspacing="0" style="z-index: 101; left: 1px; position: absolute; top: 2px; height: 200px; width: 606px;" >
      <tr>
                <td style="height: 24px" valign="bottom">
                    <span id="Label1" class="texto18">MANTENCION DE PARAMETROS</span></td>
            </tr>
        <td align="top" width="484"> <table width="548" height="115" border="0" cellpadding="0" cellspacing="0" class="texto13" id="Table2"
            ...
            <tr> 
              <td width="100" style="width: 95px; height: 18px">&nbsp; <span id="Lb_identificador" style="display:inline-block;width:83px;">% IVA</span></td>
              <?php 
			
			 if ($opc2_w <> "C")
			 {
			   echo '<td style="width: 314px; height: 3px"> <input name="cmd_iva" type="text" maxlength="5" class="input-normal" id="cmd_iva" size="6" onKeypress="if ((event.keyCode < 46 || event.keyCode > 57) || event.keyCode == 47 ) event.returnValue = false;" value="'.$iva_w.'" />';
			  }
			 else
			   { 
 			   echo '<td style="width: 94px; height: 18px"> <input name="cmd_iva" type="text" maxlength="5" id="cmd_iva" class="input-normal" style="text-transform: uppercase;" onKeypress="if (event.keyCode < 48 || event.keyCode > 57) event.returnValue = false;" value="'.iva_w.'" readOnly="readonly"/> ';
  
              }
			  echo '<span id="CodMalo" style="color:Red;display:none;">* Valor inválido</span> </td>';
    ...
    if ( $error_w )	
	  {
	   exit();
		...
		$sql1 = "call cam_pupd_parametros('".$modo_w."',null,null,null,'".$iva_w."')";
		...
		if (!mysqli_query($link,$sql1)) 
		   ...
		   {
			echo "<script type=\"text/javascript\">
				alert('Los parámetros han sido registrados de manera satisfactoria.');
				</script>";
			mysqli_close($link);
			}	
