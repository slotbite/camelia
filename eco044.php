...
// Sistema			: ECO
// Programa			: ECO044.PHP
// Descripcion		: Administración Exámenes Test de Esfuerzo.
...
<HTML>
	<head>
        ...
        imagen.src = "ieco/ivvpp0020.gif";
			...
			/*-------------------------*/
              window.open('eco_autoriza.php?PAGINA=' + 'eco043.php?CODIGO=' ,'eco043','width=750, height=800, status= no, resizable= yes, menubar=no, scrollbars=yes, location=no, top=100, left=220').focus();

			}
			
	        function f_Modificar(cod){
        /*-----------------------------------*/
              window.open('eco_autoriza.php?PAGINA=' + 'eco043.php?CODIGO=' + cod,'eco043','width=750, height=550, status= no, resizable= yes, menubar=no, scrollbars=yes, location=no, top=150, left=220').focus();
               }			
		...
		<TABLE id="Table1" cellSpacing="0" cellPadding="0" width="730" border="0">
			<TR>
				...
				<td vAlign="bottom" width="600">&nbsp;
					<span id="Label1" class="texto18">BUSCAR EXAMEN  <?php echo $_SESSION['nomexa_s']; ?> </span></td>
			</TR>
			<TR>
...
if ($_POST) 
{
 if (isset($_POST["cmd_buscar"])) 
    ...
    if (strlen($fecha1_w) == 0){
		$consulta="call ECO_PSEL_EXAMENES(null,'".$nom_w."','".$ape_w."','".$rut_w."',null,null,'1','".$cdes_w."')";
        }
    elseif (strlen($fecha2_w) == 0) {
		$consulta="call ECO_PSEL_EXAMENES(null,'".$nom_w."','".$ape_w."','".$rut_w."',STR_TO_DATE('".$fecha1_w."','%d/%m/%Y'),null,'1','".$cdes_w."')";

	    }
	 else {
		$consulta="call ECO_PSEL_EXAMENES(null,'".$nom_w."','".$ape_w."','".$rut_w."',STR_TO_DATE('".$fecha1_w."','%d/%m/%Y'),STR_TO_DATE('".$fecha2_w."','%d/%m/%Y'),'1','".$cdes_w."')";
	...
	echo '<TR>';
	echo '<td background="ivvpp/ivvpp0002.jpg" height="145"></td>';
	echo '	<td vAlign="top" align="center" width="600" height="145">';
    echo '		<P><table width="600" class="link10" cellspacing="0" cellpadding="3" align="Left" rules="rows" border="1" id="dgrid" style="background-color:White;border-color:#E7E7FF;border-width:1px;border-style:None;height:20px;border-collapse:collapse;">';
	echo '<tr style="color:#F7F7F7;background-color:#A55129;">';
	echo '	<td>Código</td><td>Apellidos</td><td>Nombres</td><td>Rut</td><td>Fecha</td><td>Exámen</td>';
	echo '</tr>';
	...
	while ($row=mysqli_fetch_array($result))
		{
		  ...
		  echo '<td height="15" width="30"> <a id="codigo_h" class="link10" href="javascript:f_Modificar(&quot;'.$row["codigo"].'.'.$row["cdescripcion"].'&quot;)">'.$row["codigo"].'</a> ';
		  echo '</td>';
  		  echo '<td width="30">'.$row["ppaciente"].'</td>';
		  echo '<td width="30">'.$row["npaciente"].'</td>';
  		  echo '<td width="60">'.$row["rutpaciente"].'</td>';
		  echo '<td width="20">'.$row["fecha"].'</td>';
  		  echo '<td width="150">'.$row["nExamen"].'</td>';
 ...
 if (isset($_POST["cmd_agregar"])) 
    {
//	 include ("eco003.php"); 
    $extra = 'eco_autoriza.php?PAGINA=' . 'eco043.php?CODIGO=.'.$cdes_w ;
