...
//			  alert(opc);
              ...
              if(opc!=='C')
			   ...
			   if(opc!=='I')
			    {
	          	document.Form1.Btt_aceptar.disabled = false;
			  	}	
...
//echo $página;
	...
	$select_w = "Select art_codpro,pro_nomfa,art_codart,art_nompro From cam_articulos
...
//	$result = mysqli_query("SELECT * FROM artículos WHERE visible = 1 ORDER BY fecha DESC LIMIT $inicio, $registros");
	...
	if(($página - 1) > 0) {
     echo "<a href='eco_autoriza.php?PAGINA=eco002_muestra.php?página=".($página-1)."'>< Anterior</a> ";
	}
	for ($i=1; $i<=$total_páginas; $i++){
 	  if ($página == $i) {
		  ...
		  echo "<a href='eco_autoriza.php?PAGINA=eco002_muestra.php?página=$i'>$i</a> ";
		}
    }		
	if(($página + 1)<=$total_páginas) {
		$destino_w = "eco_autoriza.php?PAGINA=eco002_muestra.php?página=".($página+1);
	...
	echo '							<td align="center"><input type="submit" name="Btt_aceptar" value="Aceptar" id="Btt_aceptar" disabled="disabled" class="boton" /></td>';
	echo '						</TR>	</TABLE>';
	echo '				</td>';
	echo '			</TR>';
	echo '		</TABLE>';
