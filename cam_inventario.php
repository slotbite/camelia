<?php
// Sistema			: CAMELIA
// Programa			: CAM_inventario.PHP
// Descripcion		: Seleción de Inventario.
...
if (!isset($_SESSION["codinv_s"])){ $_SESSION['codinv_s']="";} 

?>
		<TABLE id="Table1" cellSpacing="0" cellPadding="0" width="530" border="0">
			<TR>
		    <td vAlign="bottom" width="500">&nbsp; <span id="Label1" class="texto18">INVENTARIO</span></td>
			</TR>
...
//	$consulta="call cam_psel_locales('".$cod_w."')";
	$consulta="call cam_psel_inventario('".$cod_w."',null,null,null,null)";
	...
	while ($row=mysqli_fetch_array($result))
		...
		echo '<label for="rbnRut">'.$row["codpro"]." ".$row["codart"]." ".$row["codcol"]." ".$row["talla"].'</label>';
		echo '</td><td>'.utf8_decode($row["nomfa"])." ".utf8_decode($row["nomart"])." ".utf8_decode($row["nomcol"]).'</td>';
		echo '</tr>';
    	}
...
//	    echo 'El valor es: <strong>'.$_POST['rbnRut'].'</strong><br />';
		if ($opc_w == "2") {
			...
			$_SESSION['codinv_s'] = $_POST['rbnRut'] ;
			}
		$cod_w = $_POST['rbnRut'];
