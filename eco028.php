...
// Sistema			: ECO
// Programa			: ECO028.PHP
// Descripcion		: Ingreso Exámen EcoDoppler Arterial.
...
<head><title>EcoDoppler Arterial</title>
...
//          window.open('eco_autoriza.php?PAGINA=' + 'eco002.php' ,'Paciente','width=650, height=550, status=yes, resizable=no , menubar=yes, scrollbars=yes, location=no, top=150, left=220').focus();
	...
	/*-----------------------------------*/
        ...
        window.open('eco_autoriza.php?PAGINA=' + 'eco028_i.php?CODIGO=' + cod,'eco028_i','width=650, height=550, status= no, resizable= yes, menubar=yes, scrollbars=yes, location=no, top=150, left=220').focus();
...
if (isset($_POST['txt_detalle'])){
	...
	$detalle_w  = "Se exploran los troncos arteriales de ambas extremidades inferiores con transductor lineal de 7.5 Mhz con análisis doppler color.
	
Todas las arterias exploradas, es decir, las arterias femorales comunes, superficiales y profundas, poplíteas, tibiales anteriores y tibiales posteriores se observan con morfología y aspecto normal, con lúmenes en general conservados y amplios, excepto ligero engrosamiento intimal en el ostium de las arterias femorales profundas.

El examen doppler color revela flujos conservados con curvas espectrales Doppler trifásicos y con velocidades arteriales en rango normal
";
	}
if (isset($_POST['txt_conclusion'])){
	...
	$conclusion_w = "EXAMEN VASCULAR ARTERIAL DENTRO DE LÍMITES NORMALES.";
...
if ($_GET["CODIGO"] > 0)
	...
	$consulta="call ECO_PSEL_ECODOPARTERIAL('".$_GET["CODIGO"]."',null,null,null,null,null)";
    ...
    <table id="Table1" border="0" cellpadding="0" cellspacing="0" style="z-index: 101; left: 16px; position: absolute; top: 6px; height: 498px; width: 623px;" width="510">
      <tr>
        ...
        <td style="height: 24px" valign="bottom"> <span id="Label1" class="texto18"> 
          ECODOPPLER COLOR ARTERIAL DE LAS EXTREMIDADES INFERIORES</span></td>
            </tr>
            <tr>
                <td style="background-image: url(ivvpp0002.jpg); width: 34px; height: 24px">
                </td>
...
//		$sql1 = "call ECO_PUPD_PACIENTES('I','".$rut_w."','".$nombre_w."','".$apepat_w."','".$apemat_w."',null,null,null,null,null,null,null,null,null)";
		$sql1 = "call ECO_PUPD_ECODOPARTERIAL('".$modo_w."','".$codigo_w."','".$rutpac_w."',STR_TO_DATE('".$fecha_w."','%d/%m/%Y'),";
