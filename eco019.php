...
// Sistema			: ECO
// Programa			: ECO019.PHP
// Descripcion		: Administración Exámenes EcoPelviana Femenina.
...
<HTML>
	<head>
        ...
        imagen.src = "ieco/ivvpp0020.gif";
			...
			/*-------------------------*/
              window.open('eco_autoriza.php?PAGINA=' + 'eco020.php?CODIGO=' ,'eco020','width=750, height=800, status= no, resizable= yes, menubar=no, scrollbars=yes, location=no, top=100, left=220').focus();

			}
			
	        function f_Modificar(cod){
        /*-----------------------------------*/
              window.open('eco_autoriza.php?PAGINA=' + 'eco020.php?CODIGO=' + cod,'eco020','width=650, height=550, status= no, resizable= yes, menubar=no, scrollbars=yes, location=no, top=150, left=220').focus();
               }			
		...
		<TABLE id="Table1" cellSpacing="0" cellPadding="0" width="730" border="0">
			<TR>
				...
				<td vAlign="bottom" width="600">&nbsp;
					<span id="Label1" class="texto18">BUSCAR EXAMEN EcoPelviana Femenina</span></td>
			</TR>
			<TR>
...
if ($_POST) 
{
 if (isset($_POST["cmd_buscar"])) 
    ...
    if (strlen($fecha1_w) == 0){
		$consulta="call ECO_PSEL_ECOPELVIFEM(null,'".$nom_w."','".$ape_w."','".$rut_w."',null,null)";
        }
    elseif (strlen($fecha2_w) == 0) {
		$consulta="call ECO_PSEL_ECOPELVIFEM(null,'".$nom_w."','".$ape_w."','".$rut_w."',STR_TO_DATE('".$fecha1_w."','%d/%m/%Y'),null)";

	    }
	 else {
		$consulta="call ECO_PSEL_ECOPELVIFEM(null,'".$nom_w."','".$ape_w."','".$rut_w."',STR_TO_DATE('".$fecha1_w."','%d/%m/%Y'),STR_TO_DATE('".$fecha2_w."','%d/%m/%Y'))";
 ...
 if (isset($_POST["cmd_agregar"])) 
    {
//	 include ("eco003.php"); 
    $extra = 'eco_autoriza.php?PAGINA=' . 'eco020.php?CODIGO=' ;
