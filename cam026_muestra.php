...
// Descripcion		: Buscador de Artículos por ango de códigos.
...
<html xmlns="http://www.w3.org/1999/xhtml" >
	<head><title>Busqueda de Artículos</title>
...
//	$result = mysqli_query("SELECT * FROM artículos WHERE visible = 1 ORDER BY fecha DESC LIMIT $inicio, $registros");
	...
	echo '	<td>Cód.Prov.</td></td><td>Nombre Proveedor</td><td>Cod.Art.</td><td>Nombre Artículo</td>';
	echo '</tr>';
...
//		echo '<tr align="left" style="background-color:#F7F7F7;" >';
	...
	if(($página - 1) > 0) {
     echo "<a href='eco_autoriza.php?PAGINA=cam026_muestra.php?página=".($página-1)."'>< Anterior</a> ";
	}
	for ($i=1; $i<=$total_páginas; $i++){
 	  if ($página == $i) {
		  ...
		  echo "<a href='eco_autoriza.php?PAGINA=cam026_muestra.php?página=$i'>$i</a> ";
		}
    }		
	if(($página + 1)<=$total_páginas) {
		$destino_w = "eco_autoriza.php?PAGINA=cam026_muestra.php?página=".($página+1);
