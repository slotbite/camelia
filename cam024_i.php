...
//	$cod1_w = $codpro1_w;
//    $cod2_w = $codpro2_w;
	$cod1_w = STR_PAD(TRIM($codpro1_w),3,' ');
    $cod2_w = STR_PAD(TRIM($codpro2_w),3,'Z');
...
/*	
	...
	if(($página + 1) <= $total_páginas) {
		$destino_w = "eco_autoriza.php?PAGINA=cam024_i.php?página=".($página+1);
//	   	echo " <a target='mainFrame' href='".$destino_w."'>Siguiente ></a>;";
		echo "<script>location.href='".$destino_w."'</script>";
