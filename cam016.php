...
// Sistema			: CAMELIA
// Programa			: CAM016.PHP
// Descripcion		: Mantención de Habilitados.
...
<HTML>
	<head>
        ...
        imagen.src = "ivvpp/ivvpp0020.gif";
			...
			function f_cliente(){
			/*-------------------------*/
	              window.open('eco_autoriza.php?PAGINA=' + 'cam017.php?COD=' ,'cam017','width=450, height=350, status= no, resizable= no, menubar=no, scrollbars=yes, location=no, top=150, left=220').focus();
			}
			
			function Modificar_Vtna()
	          {
 	              window.open('eco_autoriza.php?PAGINA=' + 'cam017.php?COD=' + document.getElementById('hdCodigo').Value ,'cam017','width=450, height=350, status= no, resizable= yes, menubar=no, scrollbars=yes, location=no, top=150, left=220').focus();
	          }

			function Eliminar_Vtna()
			/*-------------------------*/
	          {
 	              window.open('eco_autoriza.php?PAGINA=' + 'cam017_e.php?COD=' + document.getElementById('hdCodigo').Value ,'cam017_e','width=450, height=350, status= no, resizable= yes, menubar=no, scrollbars=yes, location=no, top=150, left=220').focus();
	          }
		...
		<TABLE id="Table1" cellSpacing="0" cellPadding="0" width="530" border="0">
			...
			<TR>
				<td align="left" width="500" height="6">
					<TABLE class="texto13" id="Table2" cellPadding="0" width="591" border="0">
        <TR> 
						<td width="85" style="WIDTH: 70px; HEIGHT: 12px"></td>
						<td width="224" style="WIDTH: 224px; HEIGHT: 12px"></td>
						<td width="178" style="WIDTH: 70px; HEIGHT: 12px"></td>
						<td width="94" style="HEIGHT: 12px"></td>
					  </TR>
					  <TR> 
						<td style="WIDTH: 85px; HEIGHT: 3px">&nbsp;Cód. Habilitado</td>
						<td style="WIDTH: 224px; HEIGHT: 3px"><input name="cmd_codhab" type="text" id="cmd_codhab" class="input-normal" style="width:76px;" value="<?php if (isset($_SESSION['codhab_s'])){ echo $_SESSION['codhab_s']; } ?>"/></td>
...
if ($_POST) 
{
 if (isset($_POST["cmd_buscar"])) 
	...
	$cod_w = $_POST["cmd_codhab"];
	
	//Conexion con la base
	$link = mysqli_connect(DB_SERVER,DB_USER,DB_PASS,DB_NAME);
	
	//Ejecutamos la sentencia SQL
	$consulta="call cam_psel_habilitados('".$cod_w."',null)";
	...
	while ($row=mysqli_fetch_array($result))
		...
		echo '<td></td><td>';
		echo '<input type="radio" name="rbnRut" value="'.$row["codhab"].'" onClick="Remueve_Opcion2(&quot;'.$row["codhab"].'&quot;,&quot;'.$opc_w.'&quot;)"/>';
		echo '<label for="rbnRut">'.$row["codhab"].'</label>';
